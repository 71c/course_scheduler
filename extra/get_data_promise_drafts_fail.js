...
// function refresh_terms(response) {
//     /* gets, loads, and saves */
//     let body = rp({
//         headers: {'user-agent': 'node.js'},
//         url: TERMS_URL,
...
function load_course_data(terms, refresh=false) {
    ...
    // Load terms into memory from file. If the file does not exist, try to get it from the server, load terms into memory, and save terms
    let get_data_promise;
    if (fs.existsSync(TERMS_PATH) && !refresh) {
        get_data_promise = new Promise((resolve, reject) => {
            const termsString = fs.readFileSync(TERMS_PATH);
            models.term_to_code = JSON.parse(termsString);
            resolve();
        });
        ...
        // get terms from web
        get_data_promise = get_response_if_first().then(() => {
            return rp({
                headers: {'user-agent': 'node.js'},
                url: TERMS_URL,
                header: response.headers,
    ...
    return get_data_promise.then(() => {
        ...
        const functions_to_execute = terms.map(function(term) {
            ...
            return new Promise((resolve, reject) => {
                ...
                const get_courses = new Promise((resolve, reject) => {
                    if (fs.existsSync(courses_path) && !refresh) {
                        fs.promises.readFile(courses_path).then(coursesString => {
                        ...
                        get_response_if_first().then(() => {
                            ...
                            console.log(`got courses data for ${term}`);
                            let courses = body.searchResults;
                ...
                const get_subjects = new Promise((resolve, reject) => {
                    if (fs.existsSync(subjects_path) && !refresh) {
                        fs.promises.readFile(subjects_path).then(subjectsString => {
                            let subjects = JSON.parse(subjectsString);
                            resolve(subjects);
                        });
                    } else {
                        get_response_if_first().then(() => {
                ...
                // get courses and subjects, wait until we got both
                Promise.all([get_courses, get_subjects]).then(values => {
                    const courses = values[0];
                    const subjects = values[1];
                    // when we got courses and subjects, put the courses in memory in our format
                    save_data_new(term, courses, subjects);
                    console.log("HYE")
                    return Promise.resolve("HYE!");
                }).catch(err => {
                    console.log(err);
                    console.log("THIS IS A SPECIAL DAY");
                    if (err.error.code === "ENOTFOUND") {
                        console.log("can't get data from internet!");
                    } else {
                        console.error(err);
        ...
        // wait for them all to finish
        return Promise.all(functions_to_execute).then(vals => {
            console.log(vals);
